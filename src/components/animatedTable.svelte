<script lang="ts">
	...
	$: showingAll = showRows.length == rows.length && showRows.length != cutoffRows;
...
<div class="container" bind:this={containerElem}>
	<div class="message">
		Showing {showingAll ? 'all ' : ''}{showRows.length} row{showRows.length == 1
			? ''
			: 's'}{showingAll ? '' : ` out of ${rows.length}${rows.length == cutoffRows ? '+' : ''}`}
		{#if expanded && showRows.length != rows.length}
			<button on:click={() => dispatch('expand')}>Show more</button>
		{/if}
	</div>
	{#if shouldAnimate && asCsv}
	...
	{/if}
	<table bind:this={tableElem} style:visibility={asCsv && shouldAnimate ? 'hidden' : 'visible'}>
		{#each showRows as row, i}
			<tr>
				{#each row as field}
					{#if i == 0}
